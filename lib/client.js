	...
	factory: (require) => {
		...
		function renderHighlight(text, q) {
			...
			while (at !== -1) {
				...
				parts.push(jsx("mark", { key: "mk" + (k++), children: text.slice(at, at + ql.length) }));
				from = at + ql.length;
...
/* 原文深搜结果面板:搜索框正下方、按钮列左侧,占满下方空间,列表内部滚动 */
.${CSS_NS}_deep{position:absolute;z-index:8;top:calc(16% + 46px);right:90px;bottom:14px;width:min(360px,calc(100% - 102px));display:flex;flex-direction:column;gap:8px;pointer-events:auto}
.${CSS_NS}_deepHead{flex:none;display:flex;align-items:center;justify-content:space-between;gap:10px;padding:0 2px;cursor:grab;user-select:none;-webkit-user-select:none;touch-action:none}
.${CSS_NS}_deepHead:active{cursor:grabbing}
.${CSS_NS}_deepHead>span:first-child{font:600 10px/1.6 ui-monospace,Consolas,monospace;letter-spacing:2px;color:#00f0ff;text-shadow:0 0 10px rgba(0,240,255,.5)}
.${CSS_NS}_deepCount{font:9px/1.6 ui-monospace,Consolas,monospace;color:#7d8bb0}
.${CSS_NS}_deepList{flex:1 1 0;min-height:0;overflow-y:auto;display:flex;flex-direction:column;gap:10px;padding:2px 2px 6px;scrollbar-width:thin;scrollbar-color:#2b3654 transparent}
.${CSS_NS}_deepList::-webkit-scrollbar{width:6px}
.${CSS_NS}_deepList::-webkit-scrollbar-thumb{background:#2b3654;border-radius:4px}
/* 命中卡片:与光点气泡(_bubble)完全一致的样式 */
.${CSS_NS}_deepItem{flex:none;box-sizing:border-box;display:flex;flex-direction:column;gap:10px;padding:14px 16px;background:transparent;border:1px solid rgba(0,240,255,.3);border-radius:16px;cursor:pointer;color:#dfe9ff;box-shadow:0 12px 46px rgba(0,0,0,.4),inset 0 0 34px rgba(0,240,255,.05);transition:border-color .15s,box-shadow .15s}
.${CSS_NS}_deepItem:hover{border-color:rgba(0,240,255,.6)}
.${CSS_NS}_deepItemSel{border-color:#00f0ff;box-shadow:0 12px 46px rgba(0,0,0,.4),0 0 18px rgba(0,240,255,.35),inset 0 0 34px rgba(0,240,255,.05)}
.${CSS_NS}_deepHeadRow{display:flex;align-items:center;justify-content:space-between;gap:10px;flex:none}
.${CSS_NS}_deepTitle{flex:none;font:600 9px/1.5 ui-monospace,Consolas,monospace;letter-spacing:1.5px;color:#00f0ff;text-shadow:0 0 8px rgba(0,240,255,.6);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:60%}
.${CSS_NS}_deepWhen{flex:none;color:#7d8bb0;font:10px/1.5 ui-monospace,Consolas,monospace}
.${CSS_NS}_deepBody{font-size:13px;line-height:1.7;color:#e8f0ff;text-shadow:0 1px 2px rgba(0,0,0,.85);word-break:break-word;white-space:pre-wrap}
.${CSS_NS}_deepBody mark{background:rgba(255,205,120,.32);color:#ffe3a8;border-radius:3px;padding:0 1px}
.${CSS_NS}_deepFoot{display:flex;align-items:center;justify-content:space-between;gap:8px;flex:none}
.${CSS_NS}_deepSrc{font:10px/1.6 ui-monospace,Consolas,monospace;color:#8fa9dd;text-shadow:0 1px 2px rgba(0,0,0,.9);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:60%}
/* 浅色模式:信息框(气泡/命中卡片)换浅底深字,保证可读性;不改变整体主题 */
body:not([data-ds-dark-theme]) .${CSS_NS}_bubble{background:rgba(248,250,255,.94);border-color:rgba(0,150,180,.45);color:#1a2333;box-shadow:0 12px 46px rgba(15,25,45,.16),inset 0 0 34px rgba(0,180,220,.06)}
body:not([data-ds-dark-theme]) .${CSS_NS}_bubble::after{border-left-color:rgba(0,150,180,.5)}
body:not([data-ds-dark-theme]) .${CSS_NS}_bubbleRole{color:#007c96;text-shadow:none}
body:not([data-ds-dark-theme]) .${CSS_NS}_bubbleWhen{color:#5a6b8c}
body:not([data-ds-dark-theme]) .${CSS_NS}_bubbleBody{color:#1c2740;text-shadow:none}
body:not([data-ds-dark-theme]) .${CSS_NS}_bubbleSrc{color:#3f557d;text-shadow:none}
body:not([data-ds-dark-theme]) .${CSS_NS}_openBtn{color:#007c96;border-color:rgba(0,150,180,.55)}
body:not([data-ds-dark-theme]) .${CSS_NS}_openBtn:hover{border-color:#007c96;box-shadow:0 0 14px rgba(0,150,180,.3);background:rgba(0,150,180,.08)}
body:not([data-ds-dark-theme]) .${CSS_NS}_closeBtn{color:#c0395a;border-color:rgba(192,57,90,.55)}
body:not([data-ds-dark-theme]) .${CSS_NS}_closeBtn:hover{border-color:#c0395a;box-shadow:0 0 14px rgba(192,57,90,.3);background:rgba(192,57,90,.08)}
body:not([data-ds-dark-theme]) .${CSS_NS}_deepHead>span:first-child{color:#007c96;text-shadow:none}
body:not([data-ds-dark-theme]) .${CSS_NS}_deepCount{color:#5a6b8c}
body:not([data-ds-dark-theme]) .${CSS_NS}_deepItem{background:rgba(248,250,255,.94);border-color:rgba(0,150,180,.4);color:#1a2333;box-shadow:0 8px 30px rgba(15,25,45,.14),inset 0 0 26px rgba(0,180,220,.05)}
body:not([data-ds-dark-theme]) .${CSS_NS}_deepItem:hover{border-color:rgba(0,150,180,.7)}
body:not([data-ds-dark-theme]) .${CSS_NS}_deepItemSel{border-color:#0096b4;box-shadow:0 8px 30px rgba(15,25,45,.14),0 0 16px rgba(0,150,180,.3),inset 0 0 26px rgba(0,180,220,.05)}
body:not([data-ds-dark-theme]) .${CSS_NS}_deepTitle{color:#007c96;text-shadow:none}
body:not([data-ds-dark-theme]) .${CSS_NS}_deepWhen{color:#5a6b8c}
body:not([data-ds-dark-theme]) .${CSS_NS}_deepBody{color:#1c2740;text-shadow:none}
body:not([data-ds-dark-theme]) .${CSS_NS}_deepBody mark{background:rgba(255,180,40,.38);color:#5c3d00}
body:not([data-ds-dark-theme]) .${CSS_NS}_deepSrc{color:#3f557d;text-shadow:none}
body:not([data-ds-dark-theme]) .${CSS_NS}_bubble,body:not([data-ds-dark-theme]) .${CSS_NS}_deepList{scrollbar-color:rgba(193,203,219,.9) transparent}
body:not([data-ds-dark-theme]) .${CSS_NS}_bubble::-webkit-scrollbar-thumb,body:not([data-ds-dark-theme]) .${CSS_NS}_deepList::-webkit-scrollbar-thumb{background:#ffffff;border:1px solid #c9d4e5;border-radius:4px}
@media (max-width: 760px){.${CSS_NS}_bubble{right:12px;left:12px;width:auto}.${CSS_NS}_deep{right:12px;width:min(340px,calc(100vw - 24px))}}
`;
		...
		const zh = {
			...
			"stat.sessions": "来源会话",
			"stat.today": "今日新增",
			"search.ph": "搜索对话原文,定位命中片段…",
			"search.deep": "原文检索",
			"search.scanning": "扫描归档中…",
			"search.none": "◈ 原文无匹配 ◈",
			"search.total": "{n} 处命中",
			"filter.all": "全部",
			"filter.user": "提问",
		...
		const en = {
			...
			"stat.today": "TODAY",
			"search.ph": "Search conversation text…",
			"search.deep": "DEEP SEARCH",
			"search.scanning": "Scanning archives…",
			"search.none": "◈ NO TEXT MATCH ◈",
			"search.total": "{n} hits",
			"filter.all": "All",
		...
		function MemoryPanel({ t, sessions, onClose }) {
			...
			const [deepResults, setDeepResults] = useState(null); // null=未触发原文深搜
			const [deepTotal, setDeepTotal] = useState(0);
			const [deepPos, setDeepPos] = useState({ x: 0, y: 0 }); // 面板拖动偏移
			const deepDragRef = useRef(null); // { px, py, x, y } 拖动起点快照
			...
			// 原文深搜:≥2 字符防抖 300ms 调用 /dsh-memory/search
			useEffect(() => {
				const q = query.trim();
				if (q.length < 2) { setDeepResults(null); setDeepTotal(0); setDeepBusy(false); setDeepPos({ x: 0, y: 0 }); return; }
				setDeepBusy(true);
				const timer = setTimeout(async () => {
					try {
						const data = await api("search", { query: q, limit: 200 });
						setDeepResults(data.entries || []);
			...
			// 浏览态:即时本地过滤(仅搜摘要);输入≥2 字符切换到后端原文深搜
			const qTrim = query.trim();
			const deepActive = qTrim.length >= 2;
			const all = entries || [];
			const list = useMemo(() => {
				if (deepActive) return deepResults || [];
				const q = qTrim.toLowerCase();
				if (q === "") return all;
				return all.filter((e) => e.text.toLowerCase().includes(q) || String(e.sessionTitle || "").toLowerCase().includes(q));
			}, [all, qTrim, deepActive, deepResults]);
			...
			// 安全跳转:仅当会话确实存在于会话列表时才 open,绝不误建新会话
			const openSessionById = useCallback((sessionId) => {
				...
				try {
					...
					if (snap && snap.byId && snap.byId[sessionId]) known = true;
				} catch {}
				if (!known) {
					...
					jsxs("div", {
						...
						children: [
							...
							error === "" && list.length === 0 && jsxs("div", {
								...
								children: [
									jsx("div", { className: CSS_NS + "_stateIcon", children: "◈" }),
									jsx("div", { children: entries === null ? t("loading") : (all.length === 0 ? t("empty") : (deepActive ? t("search.none") : t("noMatch"))) }),
									jsx("div", { children: entries === null ? "" : (all.length === 0 ? t("emptyHint") : (deepActive && deepBusy ? t("search.scanning") : "")) })
								]
							}),
							jsx(EarthScene, {
								t,
							...
							deepActive && jsxs("div", {
								className: CSS_NS + "_deep",
								style: { transform: "translate(" + deepPos.x + "px," + deepPos.y + "px)" },
								children: [
									jsxs("div", {
										className: CSS_NS + "_deepHead",
										onPointerDown: (e) => {
											deepDragRef.current = { px: e.clientX, py: e.clientY, x: deepPos.x, y: deepPos.y };
											try { e.currentTarget.setPointerCapture(e.pointerId); } catch {}
										},
										onPointerMove: (e) => {
											const d = deepDragRef.current;
											if (!d) return;
											setDeepPos({ x: d.x + (e.clientX - d.px), y: d.y + (e.clientY - d.py) });
										},
										onPointerUp: () => { deepDragRef.current = null; },
										onPointerCancel: () => { deepDragRef.current = null; },
										children: [
											jsx("span", { children: t("search.deep") }),
											jsx("span", { className: CSS_NS + "_deepCount", children: deepBusy ? t("search.scanning") : t("search.total").replace("{n}", String(deepTotal)) })
										]
									}),
									jsx("div", {
										className: CSS_NS + "_deepList",
										children: (deepResults || []).map((hit) => jsxs("div", {
											key: hit.id,
											className: CSS_NS + "_deepItem" + (selectedId === hit.id ? " " + CSS_NS + "_deepItemSel" : ""),
											onClick: () => setSelectedId((cur) => (cur === hit.id ? null : hit.id)),
											children: [
												jsxs("div", {
													className: CSS_NS + "_deepHeadRow",
													children: [
														jsx("span", { className: CSS_NS + "_deepTitle", children: "◈ " + ellipsis(String(hit.sessionTitle || t("session.unknown")), 20) }),
														jsx("span", { className: CSS_NS + "_deepWhen", children: fmtTime(hit.time) })
													]
												}),
												jsx("div", { className: CSS_NS + "_deepBody", children: renderHighlight(ellipsis(hit.text, 260), qTrim) }),
												jsxs("div", {
													className: CSS_NS + "_deepFoot",
													children: [
														jsx("span", { className: CSS_NS + "_deepSrc", children: String(hit.sessionId).slice(0, 10) + " · " + (hit.role === "user" ? t("role.user") : t("role.assistant")) }),
														jsx("button", { type: "button", className: CSS_NS + "_openBtn", onClick: (e) => { e.stopPropagation(); openSessionById(hit.sessionId); }, children: t("openSession") })
													]
												})
											]
										}))
							...
							jsx(DetailOverlay, {
								...
								onOpen: () => {
									if (selected) openSessionById(selected.sessionId);
								}
							})
						]
					})
